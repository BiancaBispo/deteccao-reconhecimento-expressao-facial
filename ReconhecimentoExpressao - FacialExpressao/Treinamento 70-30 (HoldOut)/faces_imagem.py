...
recognizer.read("trainner_70-30_FacialExpressao.xml") 
...
#Carregar imagem (colocar o nome da imagem e o formato)
image = cv.imread('TristeJS1.jpg')

#Colocar a imagem em cinza
gray = cv.cvtColor(image, cv.COLOR_BGR2GRAY)

#Para cada rostos detectados (classificar)
...
for (x, y, w, h) in faces:
    ...
    cv.imshow("Rosto Detectado", roi_gray)

    #Colocar nome na borda do detector
    id_, conf = recognizer.predict(roi_gray)
    name = labels[id_]
    #esse if é o delimitador da quantidade de pixels para assim inserir o nome, se for muito baixo não vai identificar
    if conf >= 0 and conf <= 1000:
